...
static bool hexValidator(unicode_t ch) {
  return IS_HEX(ch);
}

static bool octalValidator(unicode_t ch) {
  return IS_OCTAL(ch);
}

static bool binaryValidator(unicode_t ch) {
  return (ch == '0' || ch == '1');
}

long ACJsParser::scanNumberByShiftCount(
    int shift_count, bool (*validate)(unicode_t)) {
  long long value = 0;
  unicode_t ch;
  int char_len = 0;
