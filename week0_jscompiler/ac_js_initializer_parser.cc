...
PBaseNode ACAstBuilder::parseArrayInit() {
  ...
  while (!IS_EOF() && ptoken_->type_ != TK_SQUARE_BRACKET_RIGHT) {
    switch (ptoken_->type_) {
      case TK_OP_COMMA:
        nextToken();
        elements.push_back(PBaseNode(MAKE_VARIANT(TK_UNDEFINED)));
        break;
      case TK_OP_SPREAD:
      ...
      default:
        elements.push_back(prepareInheritCoverGrammar(
...
PBaseNode ACAstBuilder::parseObjectPropertyKey() {
  ...
  switch (type) {
    ...
    case TK_SQUARE_BRACKET_LEFT:
      ...
      EXPECT_OR_FAIL_WITH(TK_SQUARE_BRACKET_RIGHT,
          ERR_INVALID_PROPERTY_KEY, "closing square bracket ']' expected", expr);
      break;
    default:
      if (IS_KEYWORD(type)) {
      ...
      AC_COMPILER_FAIL(ptoken, ERR_INVALID_PROPERTY_KEY,
          "Invalid object key");
  }
...
PBaseNode ACAstBuilder::parseObjectProperty(bool has_proto) {
  ...
  if (ptoken_->type_ == TK_OP_COLON) {
      ...
      expr.reset(MAKE_OBJECT_INIT_PROPERTY(key, value, /* is runtime = */ is_runtime,
