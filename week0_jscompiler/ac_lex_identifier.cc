...
unicode_t ACJsParser::scanEscapedIdentifier(std::wstring* converted, bool start) {
  unicode_t ch = CHEW();
  while (!IS_EOF()) {
    if (ch != '\\') {
      break;
    ...
    SWALLOW();  // skip `u' or `x'
    ch = scanHexEscape(/* length= */ ((ch == 'u') ? 4 : 2));  // format of \uXXXX,\xXX
    if (ch < 0 || ch == '\\' ||
        (start && !IS_IDENTIFIER_START(ch)) || !IS_IDENTIFIER_PART(ch)) { 
...
PTOKEN ACJsParser::scanIdentifier() {
  ...
  if (converted.length() == 1) {
    ...
    type = TK_NULL;
  } else if (converted.compare(L"undefined") == 0) {
    type = TK_UNDEFINED;
  } else if (converted.compare(L"true") == 0) {
    type = TK_TRUE;
  } else if (converted.compare(L"false") == 0) {
    type = TK_FALSE;
  } else if (converted.compare(L"NaN") == 0) {
    type = TK_NAN;
  } else if (converted.compare(L"Infinity") == 0) {
    type = TK_INFINITY;
  } else {
    type = TK_IDENTIFIER;
  }
