  ...
  if (continue_without_label) {
    ...
    if (MATCH(TK_IDENTIFIER)) {
      ...
      if (state_.labels_.find(label->reference_name_) == state_.labels_.end()) {
        AC_COMPILER_FAIL(ptoken_, ERR_INVALID_BREAK_OR_CONTINUE, "invalid label");
      }
    }
    skipSemicolon();
...
PBaseNode ACAstBuilder::parseForStmt() {
  ...
  if (MATCH(TK_SEMICOLON)) {
    ...
    switch (lex_type) {
      ...
      default:           // x = init_value, ...
        state_.allow_in_op_ = false;
        init_left = prepareInheritCoverGrammar(
  ...
  return PBaseNode(MAKE_FOR_STATEMENT(init_left, condition_var, update_var,
      body_stmt));
}
...
PBaseNode ACAstBuilder::parseIfElseStmt() {
  ...
  return PBaseNode(MAKE_IF_ELSE_STATEMENT(condition, true_clause, false_clause));
}
...
PBaseNode ACAstBuilder::parseReturnStmt() {
  ...
  if (MATCH(TK_IDENTIFIER)) {
    ...
    if (!MATCH(TK_SEMICOLON) &&
        !MATCH(TK_CURLY_BRACKET_RIGHT) && !IS_EOF()) {
      stmt = parseExpr();
    }
    skipSemicolon();
  }
...
PCaseClause ACAstBuilder::parseCaseClause() {
  ...
  while (!IS_EOF()) {
    if (MATCH(TK_CURLY_BRACKET_RIGHT) || MATCH(TK_RSV_DEFAULT) || MATCH(TK_RSV_CASE)) {
      break;
    }
...
PBaseNode ACAstBuilder::parseTryCatchStmt() {
  ...
  return PBaseNode(MAKE_TRY_CATCH_STATEMENT(
      try_clause, catch_clause, finally_clause));
}
