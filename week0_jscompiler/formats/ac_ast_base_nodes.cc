...
static const char* SUB_NODE_ERR_NAMES[] = {"(unknown)", "token error",
                                           "syntax error"};

static const char* SUB_NODE_EXPR_NAMES[] = {
    "(unknown)",     "array expr",      "arrow expr",       "assignment expr",
    "binary expr",   "call expr",       "class expr",       "conditional expr",
    "formal list",   "func expr",       "group expr",       "member expr",
    "new expr",      "object expr",     "placeholder expr", "postfix expr",
    "sequence expr", "tagged tpl expr", "this expr",        "unary expr",
    "var decl"};

static const char* SUB_NODE_FUNC_CLASS_NAMES[] = {
    "(unknown)", "arraw expr formal", "function", "class",
    "id param",  "col param",         "obj param"};

static const char* SUB_NODE_VAR_NAMES[] = {
    "(unknown)", "string var",    "numeric var", "bool var",
    "regex var", "reference var", "special var", "variable var"};

static const char* SUB_NODE_STMT_NAMES[] = {
    "(unknown)",     "block stmt",     "break stmt",   "continue stmt",
    "debugger stmt", "do-while stmt",  "empty stmt",   "expr stmt",
    "for stmt",      "for in stmt",    "if-else stmt", "labeled stmt",
    "lex decl list", "return stmt",    "switch stmt",  "case clause",
    "throw stmt",    "try-catch stmt", "catch clause", "var decl list",
    "while stmt",    "with stmt"};

static const char* SUB_NODE_ELEMENT_NAMES[] = {
    "(unknown)", "spread element", "rest element", "template element"};

static const char* SUB_NODE_MEMBER_REF_NAMES[] = {"(unknown)", "static member",
                                                  "runtime member"};

static const char* SUB_NODE_OBJECT_NAMES[] = {"(unknown)", "general object",
                                              "object init prop"};
...
ACAstBaseNode::ACAstBaseNode(PPOSITION pos, const AstNodeType type,
...
#define BASE_NODE_CONSTRUCT_IMPL(node, base)                                  \
  ACAst##node##Node::ACAst##node##Node(PPOSITION pos, const AstNodeType type, \
                                       const AstNodeSubType sub_type)         \
      : ACAst##base##Node(std::move(pos), type, sub_type) {}

void ACAstBaseNode::dumpWString(std::ostream& os, const std::wstring& literal) {
  for (size_t i = 0; i < literal.length(); ++i) {
    os << "[" << literal.at(i) << "]";
  }
}

void ACAstBaseNode::dumpChildNodeGraph(std::ostream& os, const PBaseNode& child,
...
void ACAstBaseNode::dumpSubNodes(std::ostream& os, const char* begin_wrapper,
                                 const char* end_wrapper,
...
/////////////////////////////////////////////////////////////////////////////////////////
Value* ACAstBaseNode::buildAst() { return nullptr; }
...
void ACAstBaseNode::dumpGraphLabel(std::ostream& os) const { os << "?"; }
...
void ACAstBaseNode::dumpGraph(std::ostream& os) const {
  ...
  os << " [label=\"" << NODE_NAMES[node_type_] << ", "
     << (node_sub_type_ != AST_SUB_GENERAL
             ? SUB_NODE_NAMES[node_type_][node_sub_type_ % SUB_NODE_MAX_SIZE]
             : "general")
     << "\n";
  dumpGraphLabel(os);
  os << "\"];\n";
...
/////////////////////////////////////////////////////////////////////////////////////////
ACAstExpressionNode::ACAstExpressionNode(PPOSITION pos, PBaseNode expr,
...
/////////////////////////////////////////////////////////////////////////////////////////
ACAstStatementNode::ACAstStatementNode(PPOSITION pos, PBaseNode stmt,
                                       const AstNodeType type,
                                       const AstNodeSubType sub_type)
    : ACAstBaseNode(std::move(pos), type, sub_type), stmt_(std::move(stmt)) {}
...
/////////////////////////////////////////////////////////////////////////////////////////
ACAstBlockNode::ACAstBlockNode(PPOSITION pos, PBaseNodeList stmt_list,
                               const AstNodeType type,
                               const AstNodeSubType sub_type)
...
/////////////////////////////////////////////////////////////////////////////////////////
ACAstCollectionNode::ACAstCollectionNode(PPOSITION pos, PBaseNodeList elements,
                                         const AstNodeType type,
                                         const AstNodeSubType sub_type)
    : ACAstBaseNode(std::move(pos), type, sub_type) {
