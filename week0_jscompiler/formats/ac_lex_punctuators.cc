    ...
    TK_INVALID,
    {
        PTN{'!',
            ...
            {// !    : 11.4.9
             PTN{
                 '=',
                 TK_OP_NOT_EQUAL_V,  // !=   : 11.9.2
                 {PTN{'=', TK_OP_NOT_EQUAL_VT, {/* end node */}}}  // !==
                                                                   // : 11.9.5
             }}},                                                  // PTN '!'
...
static ACLexTokenType searchTrie(const PunctuatorTrieNode& node,
                                 const wchar_t* search_key,
                                 int* remaining_length) {
  ACLexTokenType type = node.value_;
  if (*remaining_length > 0 && !node.nodes_.empty()) {
    NodeIterator it = std::lower_bound(node.nodes_.begin(), node.nodes_.end(),
                                       search_key, CompareTrieKey);
