...
ACAstBaseError::ACAstBaseError(PPOSITION pos, const AstNodeSubType sub_type,
                               const ACLexTokenType type)
    : ACAstBaseNode(std::move(pos), AST_ERR, sub_type), token_type_(type) {}

ACAstBaseError::ACAstBaseError(PPOSITION pos, const AstNodeSubType sub_type,
                               PBaseNode error_node)
    : ACAstBaseNode(std::move(pos), AST_ERR, sub_type),
      error_node_(std::move(error_node)) {}

void ACAstBaseError::dump(std::ostream& os) const { ACAstBaseNode::dump(os); }
