  ...
  while (!IS_EOF()) {
    ...
    switch (ch) {
      ...
      case 'x':
        ...
        break;
      case 'n':
        SWALLOW(); literal.append(1, '\n'); break;
      case 'r':
        SWALLOW(); literal.append(1, '\r'); break;
      case 't':
        SWALLOW(); literal.append(1, '\t'); break;
      case 'b':
        SWALLOW(); literal.append(1, '\b'); break;
      case 'f':
        SWALLOW(); literal.append(1, '\f'); break;
      case 'v':
        SWALLOW(); literal.append(1, 0xb); break;
      case '\\':
      case '\'':
      case '\"':
      case '/':
        SWALLOW(); literal.append(1, UNICODE_TO_WCHAR(ch)); break;
      case '0':
      case '1':
      case '2':
      case '3':
      ...
      case '7':
        ...
        if (IS_OCTAL(next_ch)) {
          ...
          if (IS_OCTAL(next_next_ch)) {
            literal.append(1, UNICODE_TO_WCHAR(
                MAKE_OCTAL(ch, next_ch, next_next_ch)));
            SKIP();
            break;
          } else if (ch == '0') {
            literal.append(1, '\0');
