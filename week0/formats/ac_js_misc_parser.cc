...
PBaseNodeList ACAstBuilder::parseScriptBody(bool push_state, bool in_function,
                                            bool expect_bracket) {
...
bool ACAstBuilder::skipSemicolon(bool ignore_error) {
  ...
  if (!MATCH(TK_EOF) && !MATCH(TK_CURLY_BRACKET_RIGHT) && !ignore_error) {
    AC_COMPILER_FAIL_WITH(ptoken_, ERR_INVALID_TOKEN,
                          "Unexpected token while semicolon is expected",
                          false);
  }
  return true;
}
...
PBaseNodeList ACAstBuilder::parseFormalList(PBaseNode expr) {
  ...
  for (PBaseNodeList::iterator iter = formal_list.begin();
    ...
    PBaseNode& node = *iter;
    if (!!node &&
        node->node_sub_type_ == ACAstBaseNode::AST_ASSIGNMENT_EXPRESSION) {
...
PBaseNode ACAstBuilder::parseTokenAsVariant(ACLexToken* ptoken,
                                            bool for_object_key) {
  PBaseNode expr;
  switch (ptoken->type_) {
    case TK_STRING:
    ...
    case TK_NULL:
      ...
      break;
    default:
      AC_COMPILER_FAIL(PTOKEN(ptoken), ERR_INVALID_TOKEN,
                       "Cannot parse token as variant");
  }
  return expr;
}
  ...
  if (ptoken_->type_ != TK_PAREN_RIGHT) {
    while (!IS_EOF()) {
      ...
      EXPECT_OR_FAIL_WITH(TK_OP_COMMA, ERR_INVALID_ARGUMENT_LIST,
                          "Missing ',' in between the argument list",
                          arguments);
    }
  }
...
PBaseNode ACAstBuilder::parseStaticProperty() {
  ...
  // Starting ECMAScript5, reserved keyword is allowed to be static property
  // name. e.g. "Promise.prototype.catch".
  if (!(MATCH_TYPE(token->type_, TK_IDENTIFIER) || IS_KEYWORD(token->type_))) {
    AC_COMPILER_FAIL(ptoken_, ERR_INVALID_MEMBER_IDENTIFIER,
                     "Invalid identifier of a member");
  }
