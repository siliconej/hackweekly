...
//                Also need to show what are the list of tokens expected at this
//                context.
PBaseNode ACAstBuilder::recoverFromError(const PTOKEN& ptoken) {
  ...
  return make_unique<ACAstTokenError>(std::move(pos),
                                      (!!ptoken ? ptoken->type_ : TK_INVALID));
}

PBaseNode ACAstBuilder::recoverFromError(PBaseNode error_node) {
  PBaseNode empty_err_node;
  // FIXME(me): the syntax error position is empty for now, determine the right
  // position.
  PPOSITION pos(make_unique<ACLexPosition>().release());
  if (!!error_node) {
    return make_unique<ACAstSyntaxError>(std::move(pos), std::move(error_node));
  } else {
    return make_unique<ACAstSyntaxError>(
        std::move(pos),
