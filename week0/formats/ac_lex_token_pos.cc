...
ACLexToken::ACLexToken() : type_(TK_INVALID), len_(0), orig_literal_("") {}

ACLexToken::ACLexToken(ACLexTokenType t)
    : type_(t), len_(0), orig_literal_("") {}

ACLexToken::ACLexToken(ACLexTokenType t, size_t l, std::string_view raw,
                       ACLexPosition& p)
...
ACLexToken::ACLexToken(ACLexTokenType t, size_t l, std::string_view raw,
                       ACLexPosition& p, const wchar_t* converted)
    : type_(t),
      len_(l),
      orig_literal_(raw),
      pos_(p),
      unicode_literal_(converted) {}
...
void ACLexToken::dump(std::ostream& os) const {
  os << "{Type(" << type_ << ")=" << ACLexToken::TOKEN_NAMES[type_]
     << ", Pos=" << pos_ << ", len=" << len_ << ", `" << orig_literal_ << "'"
     << "(unicode len=" << unicode_literal_.length();
...
ACLexPosition::ACLexPosition() : pos_(0), line_num_(1), line_pos_(0) {}
...
ACLexRegexToken::ACLexRegexToken(size_t l, std::string_view raw,
                                 ACLexPosition& p, PLEXREGEX regex)
