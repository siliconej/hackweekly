...
PBaseNode ACAstBuilder::parseBinaryExpr() {
  ...
  operations.push_back(PBinaryExpr(
      MAKE_PENDING_BINARY_EXPRESSION(binary_op, precedence)));
  ...
  while (true) {
    ...
    while (operations.size() > 2 &&
           ...
           // shrink them first.
           precedence <=
               operations[operations.size() - 2]->precedence()) {
       // +-----------Stack top-------------
       //   top() -> rhs
       //   top() - 1 -> binary_expr
       //   rhs -> binary_expr->rhs
    ...
    operations.push_back(prepareIsolateCoverGrammar(&ACAstBuilder::parseUnaryExpr));
  }
...
PBaseNode ACAstBuilder::parseConditionalExpr() {
  ...
  if (MATCH(TK_OP_QUESTION_MRK)) {
    ...
    PBaseNode true_expr = prepareIsolateCoverGrammar(&ACAstBuilder::parseAssignmentExpr);
    state_.restoreAllowInState();

    EXPECT_OR_FAIL(TK_OP_COLON,
        ERR_INVALID_CONDITIONAL_EXPR, "Missing ':' in conditional expression");
    PBaseNode false_expr = prepareIsolateCoverGrammar(&ACAstBuilder::parseAssignmentExpr);

    expr.reset(MAKE_CONDITIONAL_EXPRESSION(
        expr, std::move(true_expr), std::move(false_expr)));
    ASSIGNMENT_AND_BINDING(false, false);
  }
...
PBaseNode ACAstBuilder::parseGroupExpr() {
  ...
  EXPECT_OR_FAIL(TK_PAREN_LEFT,
      ERR_INVALID_GROUP_EXPR, "Missing '(' in group expression");

  PBaseNodeList params;
  if (MATCH(TK_PAREN_RIGHT)) {
    nextToken();
    EXPECT_OR_FAIL(TK_OP_LAMBDA,
        ERR_INVALID_LAMBDA_EXPR, "Missing '=>' in lambda expression");
    expr.reset(MAKE_PLACEHOLDER_EXPRESSION(params)); 
  } else if (ptoken_->type_ == TK_OP_SPREAD) {
    expr = parseRestElement();
    EXPECT_OR_FAIL(TK_PAREN_RIGHT,
        ERR_INVALID_SPREAD_EXPR, "Missing ')' in spread expression");
    EXPECT_OR_FAIL(TK_OP_LAMBDA,
        ERR_INVALID_LAMBDA_EXPR, "Missing '=>' in lambda expression");
    ...
    if (MATCH(TK_OP_COMMA)) {
      ...
      while(!IS_EOF()) {
        ...
        if (MATCH(TK_OP_SPREAD)) {
          if(!cur_builder_state_->is_binding_element_) {
            AC_COMPILER_FAIL(ptoken_,
                ERR_INVALID_SPREAD_EXPR,"not a binding element");
          }
        ...
        exprs.push_back(prepareInheritCoverGrammar(&ACAstBuilder::parseAssignmentExpr));
...
PBaseNode ACAstBuilder::parseLhsExpr(bool allow_call) {
  ...
  if (allow_call) {
    ...
    AC_COMPILER_ASSERT_C(state_.allow_in(),
        ptoken_, ERR_INTERNAL_INVALID_ALLOW_IN, "Invalid Allow In status");
  }
  ...
  if (MATCH(TK_RSV_SUPER) && state_.in_function()) {
    ...
    expr = prepareInheritCoverGrammar((MATCH(TK_RSV_NEW) ?
        &ACAstBuilder::parseNewExpr : &ACAstBuilder::parsePrimaryExpr));
  }
  ...
  while (!finished) {
    ...
    switch (type) {
      case TK_SQUARE_BRACKET_LEFT:  // [
        ...
        expr.reset(MAKE_RUNTIME_MEMBER_REF(expr,
...
PBaseNode ACAstBuilder::parseNewExpr() {
  ...
  EXPECT_OR_FAIL(TK_RSV_NEW,
      ERR_INVALID_NEW_STATEMENT, "Missing new keyword in new statement");
  PBaseNode expr = prepareIsolateCoverGrammar(&ACAstBuilder::parseLhsNoCallExpr);
  PBaseNodeList args;
...
PBaseNode ACAstBuilder::parsePostfixExpr() {
  ...
  PBaseNode expr = prepareInheritCoverGrammar(&ACAstBuilder::parseLhsAllowCallExpr);
  const ACLexTokenType type = ptoken_->type_;

  if (!has_linebreak_ && (type == TK_OP_SELF_PLUS || type == TK_OP_SELF_MINUS)) {
    // FIXME(ejiang): support strict  mode.
    AC_COMPILER_ASSERT_C(cur_builder_state_->is_assignment_target_,
        ptoken_, ERR_INVALID_LHS_IN_ASSIGNMENT, "Invalid postfix expression.");
    ASSIGNMENT_AND_BINDING(false, false);
    nextToken();
...
PBaseNode ACAstBuilder::parsePrimaryExpr() {
  ...
  switch (type) {
    ...
    case TK_TRUE:        // true
    case TK_FALSE:       // false
    case TK_UNDEFINED:   // undefined
    case TK_NULL:        // null
    case TK_NAN:         // NaN
    case TK_INFINITY:    // Infinity
    ...
    switch (type) {
      ...
      case TK_OP_SELF_MINUS:  // --
        ...
        AC_COMPILER_ASSERT_C(cur_builder_state_->is_assignment_target_, ptoken_,
            ERR_INVALID_LHS_IN_ASSIGNMENT, "Invalid unary expression.");
...
PBaseNode ACAstBuilder::parseAssignmentExpr() {
  ...
  if (expr->node_sub_type_ == ACAstBaseNode::AST_PLACEHOLDER_EXPRESSION ||
    ...
    AC_COMPILER_ASSERT_C(cur_builder_state_->is_assignment_target_ &&
        expr->node_type_ != ACAstBaseNode::AST_SYN_LITERAL, ptoken_,
        ERR_INVALID_LHS_IN_ASSIGNMENT, "Invalid assignment operation in LHS");
    ...
    nextToken();
    expr.reset(MAKE_ASSIGNMENT_EXPRESSION(expr, type,
...
PBaseNode ACAstBuilder::parseExpr() {
  ...
  PBaseNode expr = prepareIsolateCoverGrammar(&ACAstBuilder::parseAssignmentExpr);
  ...
  if (MATCH(TK_OP_COMMA)) {
    ...
    while (!IS_EOF() && MATCH(TK_OP_COMMA)) {
       nextToken();
       expressions.push_back(prepareIsolateCoverGrammar(&ACAstBuilder::parseAssignmentExpr));
    }
