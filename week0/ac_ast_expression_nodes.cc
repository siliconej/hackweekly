...
ACAstArrowFunctionExpression::ACAstArrowFunctionExpression(
    PPOSITION pos, PBaseNode formal, PBaseNode body,
...
ACAstAssignmentExpression::ACAstAssignmentExpression(PPOSITION pos,
    PBaseNode assignee_expr, ACLexTokenType type, PBaseNode value_expr) :
  ACAstExpressionNode(std::move(pos), std::move(assignee_expr),
                      AST_EXPR, AST_ASSIGNMENT_EXPRESSION), op_type_(type) {
  sub_nodes_.push_back(std::move(value_expr));
}

Value* ACAstAssignmentExpression::acceptInternal(ACAstVisitable* v) const {
  // FIXME(ejiang): implement converted assignment types.
  if (node_type_ != AST_EXPR) {
    return nullptr;
  }
  return v->visit(this);
}
...
/////////////////////////////////////////////////////////////////////////////////
ACAstBinaryExpression::ACAstBinaryExpression(PPOSITION pos,
    ACLexTokenType type, int prec, PBaseNode lhs_expr, PBaseNode rhs_expr) :
  ACAstExpressionNode(std::move(pos), nullptr, AST_EXPR, AST_BINARY_EXPRESSION),
  op_type_(type), precedence_(prec) {
  sub_nodes_.push_back(std::move(lhs_expr));
  sub_nodes_.push_back(std::move(rhs_expr));
}

Value* ACAstBinaryExpression::acceptInternal(ACAstVisitable* v) const {
  if (sub_nodes_.size() < 2) {
    return nullptr;
  }
  // To avoid side effect, we need to do DFS instead of BFS.
  return v->visit(this);
}
...
/////////////////////////////////////////////////////////////////////////////////
ACAstCallExpression::ACAstCallExpression(PPOSITION pos,
    PBaseNode expr, PBaseNodeList args) :
  ACAstExpressionNode(std::move(pos), std::move(expr),
  AST_EXPR, AST_CALL_EXPRESSION) {
  sub_nodes_ = std::move(args);
}

Value* ACAstCallExpression::acceptInternal(ACAstVisitable* v) const {
  return v->visit(this);
}
...
/////////////////////////////////////////////////////////////////////////////////
ACAstPostfixExpression::ACAstPostfixExpression(PPOSITION pos,
    PBaseNode expr, ACLexTokenType type) :
  ACAstExpressionNode(std::move(pos), std::move(expr), AST_EXPR, AST_POSTFIX_EXPRESSION),
  op_type_(type) {}

Value* ACAstPostfixExpression::acceptInternal(ACAstVisitable* v) const {
  return v->visit(this);
}
...
/////////////////////////////////////////////////////////////////////////////////
ACAstConditionalExpression::ACAstConditionalExpression(PPOSITION pos,
    PBaseNode expr, PBaseNode true_case_expr, PBaseNode false_case_expr) :
  ACAstExpressionNode(std::move(pos), std::move(expr), AST_EXPR, AST_CONDITIONAL_EXPRESSION) {
  sub_nodes_.push_back(std::move(true_case_expr));
  sub_nodes_.push_back(std::move(false_case_expr));
}

Value* ACAstConditionalExpression::acceptInternal(ACAstVisitable* v) const {
  return v->visit(this);
}
...
/////////////////////////////////////////////////////////////////////////////////
ACAstNewExpression::ACAstNewExpression(PPOSITION pos,
    PBaseNode expr, PBaseNodeList args) :
  ACAstExpressionNode(std::move(pos), std::move(expr), AST_EXPR, AST_NEW_EXPRESSION) {
  sub_nodes_ = std::move(args);
}

Value* ACAstNewExpression::acceptInternal(ACAstVisitable* v) const {
  return v->visit(this);
}
...
ACAstPlaceHolderExpression::ACAstPlaceHolderExpression(PPOSITION pos,
  ...
  ACAstExpressionNode(std::move(pos), nullptr, AST_EXPR, AST_PLACEHOLDER_EXPRESSION) {
...
/////////////////////////////////////////////////////////////////////////////////
ACAstSequenceExpression::ACAstSequenceExpression(PPOSITION pos, PBaseNodeList exprs) :
  ACAstExpressionNode(std::move(pos), nullptr, AST_EXPR, AST_SEQUENCE_EXPRESSION) {
  sub_nodes_ = std::move(exprs);
}

Value* ACAstSequenceExpression::acceptInternal(ACAstVisitable* v) const {
  Value* result = nullptr;
  for (PBaseNodeList::const_iterator iter = sub_nodes_.begin();
           iter != sub_nodes_.end(); ++iter) {
    result = (*iter)->accept(v);
  }
  return result;
}
...
/////////////////////////////////////////////////////////////////////////////////
ACAstUnaryExpression::ACAstUnaryExpression(PPOSITION pos,
    PBaseNode expr, ACLexTokenType type) :
  ACAstExpressionNode(std::move(pos), std::move(expr), AST_EXPR, AST_UNARY_EXPRESSION),
  op_type_(type) {}

Value* ACAstUnaryExpression::acceptInternal(ACAstVisitable* v) const {
  return v->visit(this);
}
...
//// VariableDecl /////////////////////////////////////////////////////////////////
ACAstVariableDecl::ACAstVariableDecl(PPOSITION pos,
    PBaseNode param_pattern, PBaseNode init_value,
    const AstNodeType type, const AstNodeSubType sub_type) :
  ACAstExpressionNode(std::move(pos), std::move(param_pattern), type, sub_type) {
  if (!!init_value) {
    sub_nodes_.push_back(std::move(init_value));
  }
}

Value* ACAstVariableDecl::acceptInternal(ACAstVisitable* v) const {
  return v->visit(this);
}
...
ACAstFormalListExpression::ACAstFormalListExpression(
    PPOSITION pos, PBaseNodeList formal_list,
    const AstNodeType type, const AstNodeSubType sub_type) :
